<template>
  <div class="bg-white p-7 sm:p-10">
    ...
    <div class="mt-8 grid gap-3 sm:grid-cols-2">
      <button type="button" class="flex items-center justify-between border border-line px-6 py-3.5 text-sm font-medium uppercase tracking-[0.04em] text-ink transition-colors duration-fast hover:border-ink/40" @click="print">
        Download Invoice
      ...
      <button type="button" class="flex items-center justify-between border border-line px-6 py-3.5 text-sm font-medium uppercase tracking-[0.04em] text-ink transition-colors duration-fast hover:border-ink/40" @click="print">
        Print Reservation
        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 9.75V4.5a.75.75 0 01.75-.75h9.06a.75.75 0 01.75.75v5.25M6.72 18.75h-2.4a.75.75 0 01-.75-.75v-5.25a.75.75 0 01.75-.75h16.36a.75.75 0 01.75.75V18a.75.75 0 01-.75.75h-2.4M6.72 14.25h10.56v6a.75.75 0 01-.75.75H7.47a.75.75 0 01-.75-.75v-6z" />
        </svg>
      </button>
      <!-- Resend requires an email service; lands with the PMS/notifications
           integration, not something the browser can do on its own -->
      <button type="button" class="flex items-center justify-between border border-line px-6 py-3.5 text-sm font-medium uppercase tracking-[0.04em] text-ink transition-colors duration-fast hover:border-ink/40">
      ...
      <button type="button" class="flex items-center justify-between border border-line px-6 py-3.5 text-sm font-medium uppercase tracking-[0.04em] text-ink transition-colors duration-fast hover:border-ink/40" @click="downloadCalendarInvite">
        Add to Calendar
        <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
    ...
    <h3 class="mt-10 font-display text-2xl text-ink sm:text-3xl">Explore More</h3>
    <div class="mt-6 grid grid-cols-3 gap-3">
      <NuxtLink v-for="item in exploreLinks" :key="item.label" :to="item.to" class="group">
        <div class="relative aspect-[4/3] overflow-hidden bg-line/40">
          <BaseImage :src="item.image" alt="" :width="240" :height="180" sizes="33vw" class="absolute inset-0 transition-transform duration-normal ease-premium group-hover:scale-105" />
        </div>
        <p class="mt-2 flex items-center gap-1 text-sm font-medium text-ink">
          {{ item.label }}
          <svg class="h-3.5 w-3.5 text-copper" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
...
const exploreLinks = [
  { label: 'Dining', to: '/#dining', image: '/images/dining/bbq-restaurant.jpg' },
  { label: 'Spa & Wellness', to: '/#facilities', image: '/images/offers/stressless-spa.jpg' },
  { label: 'Swimming Pool', to: '/#facilities', image: '/images/offers/swimming.jpg' },
]
