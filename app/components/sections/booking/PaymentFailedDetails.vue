<template>
  <div class="bg-white p-7 sm:p-10">
    <h2 class="font-display text-2xl text-ink sm:text-[1.75rem]">Booking Details</h2>
    <div class="mt-6">
      <BookingStayDetailsGrid
        :check-in="checkIn"
        :check-out="checkOut"
        :nights="nights"
    ...
    <div class="mt-4 space-y-3 border-t border-line pt-6 text-sm">
      <div class="flex items-center justify-between text-ink/70">
        ...
        <span class="text-ink">${{ totalPrice.toLocaleString('en-US') }}</span>
      </div>
      <div class="flex items-center justify-between text-ink/70">
      ...
      <div class="flex items-center justify-between text-ink/70">
        <span>Status</span>
        <span class="flex items-center gap-1.5 text-sm font-medium text-[rgb(190,50,45)]">
          <svg class="h-4 w-4 shrink-0" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
            <path d="M8 1.333A6.667 6.667 0 1014.667 8 6.674 6.674 0 008 1.333zm2.14 8.14a.667.667 0 01-.943.943L8 9.14l-1.197 1.276a.667.667 0 01-.943-.943L7.06 8 5.86 6.803a.667.667 0 11.943-.943L8 6.86l1.197-1.196a.667.667 0 11.943.943L8.94 8z" />
          </svg>
          Payment Failed
        </span>
      </div>
    ...
    <div class="mt-8 flex items-start gap-3 border border-line bg-paper/60 p-5">
      ...
      <div>
        ...
        <p class="mt-1 text-sm text-ink/70">
          Your reservation is not confirmed, and no payment has been charged. You can try again with the same or a different payment method, or contact our team if you need assistance.
        </p>
      </div>
    </div>
  </div>
